...
def test_mobile_home_aggregation_missing():
    ...
    assert response.status_code == 200, (
        "O endpoint /mobile-home já existe? Deveria ser criado pelo aluno."
    )


def test_rate_limit_implemented():
...
def test_mobile_home_success():
    ...
    response = client.get("/mobile-home")
    assert response.status_code == 200, (
        "ERRO: O endpoint /mobile-home ainda retorna 404 ou erro."
    )
...
def test_rate_limit_blocking():
    ...
    assert 429 in status_codes, (
        f"ERRO: O Gateway não bloqueou o Bot. Status recebidos: {status_codes}"
    )
